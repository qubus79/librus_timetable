...
def setup(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv('APP_PASSWORD', 'test-family-password-long')
...
def login(c):
    assert c.post('/api/login', json={'password':'test-family-password-long'}).status_code == 200
...
def test_auth_and_csrf(setup):
    ...
    assert c.post('/api/login',json={'password':'test-family-password-long'},headers={'Origin':'https://evil.example'}).status_code==403
    assert c.post('/api/login',json={'password':'test-family-password-long'},headers={'X-Dzwonek':''}).status_code==403
    login(c)
...
def test_rate_limit(setup):
    c,m=setup
    for _ in range(10):assert c.post('/api/login',json={'password':'wrong'}).status_code==401
...
def test_children_encryption_cache_edit_delete(setup,monkeypatch):
    ...
    assert len(children)==1 and 'credentials' not in children[0]
    assert 'pass-secret' not in json.dumps(children)
    with m.db() as db:
        row=db.execute('SELECT * FROM children').fetchone()
...
def test_client_isolation(monkeypatch):
    ...
    librus.fetch_timetable('c','d','2026-09-21')
